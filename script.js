...
CrosswordHeight = 16;
Words = 15;
WordLength = new Array(9, 6, 5, 8, 6, 5, 9, 11, 13, 11, 10, 9, 8, 4, 6);
Word = new Array("АНАКСАГОР", "НОВОГО", "ДИДРО", "ФЕЙЕРБАХ", "ЛОГИКА", "ЭТИКА", "МИЛЕТСКАЯ", "ТЕОЦЕНТРИЗМ", "СРЕДНЕВЕКОВАЯ", "ГНОСЕОЛОГИЯ", "АБСОЛЮТНЫЙ", "ФИЛОСОФИЯ", "ГЕРАКЛИТ", "КАНТ", "СОКРАТ");
Clue = new Array("Фамилия философа при решении проблемы который исходил из принимаемого им в качестве закона \"о том, что ничто не возникает из небытия\".", 
                "Философии этого ВРЕМЕНИ характерны Реформации Католиков, окончательная победа антропоцентризма и смещение философии в гносеологию.", 
                " ► ГЛАВНЫЙ ВОПРОС! ◄ <br><br>Философ,а так же драматург и демокритик утверждающий, что душа - продукт единства организма, его целостности.", 
                "Фамилия немецкого философа разработавшиего концепцию антропологического материализма.", 
                "Наука изучающая законы и формы мышления.", 
                "Философское учение о морали", 
                "Древнегреческая школа, представитель которой выдвинул идею субстанции как первоосновы сущего.", 
                "Понятние, согсласно которому, в центре мира находится Бог. Он же предопределяет пути, по которым следует человек.", 
                "Этап философии распостранённый в Европе в V-XIX в, Бог здесь выступал в качестве высшего начала, а окружающий мир его творением.", 
                "Одна из дисциплин (учение) философии занимающаяся познанием.", 
                "Дух Гегеля, означающий развитую форму бытия абсолютной идем.", 
                "Термин науки впервые употреблённый и введённый в использование Платоном.", 
                "Имя философа, которому принадлежит выражение: \"В одну реку нельзя войти дважды\".", 
                "Фамилия философа, которому принадлежит максима \"поступай с другими так, как хотел бы, чтобы поступали с тобой\".", 
                "Учитель философа Платона");
AnswerHash = new Array(80102, 28409, 54555, 41587, 77917, 30364, 11331, 91923, 49714, 51541, 79232, 41105, 25444, 87391, 14042);
WordX = new Array(0, 12, 8, 11, 15, 10, 0, 0, 8, 12, 16, 1, 4, 19, 6);
WordY = new Array(1, 2, 3, 5, 7, 10, 12, 15, 0, 1, 4, 5, 5, 7, 10);
...
{
    document.getElementById("waitmessage").innerHTML = "Пожалуйста, подождите прогрузку сайта...";
...
function SelectThisWord(event)
{
    ...
    document.getElementById("wordinfo").innerHTML = ((CurrentWord <= LastHorizontalWord) ? "По горизонтали вправо, " : "По вертикали вниз, ") + WordLength[CurrentWord] + " букв.";
...
function OKClick()
{
    ...
    {
        document.getElementById("worderror").innerHTML = "Введенное вами слово содержит недопустимые символы. Пожалуйста, введите в поле только буквы.";
        document.getElementById("worderror").style.display = "block";
        return;
    }
    if (TheirWord.length < WordLength[CurrentWord])
    {
        document.getElementById("worderror").innerHTML  = "Вы ввели недостаточно букв в слове. Это слово имеет " + WordLength[CurrentWord] + " букв.";
        document.getElementById("worderror").style.display = "block";
        return;
    }
    if (TheirWord.length > WordLength[CurrentWord])
    {
        document.getElementById("worderror").innerHTML = "Слишком много букв в слове. Это слово имеет " + WordLength[CurrentWord] + " букв.";
        document.getElementById("worderror").style.display = "block";
        return;
    }
...
function CheckClick()
{
    ...
    if (ErrorsFound > 0 && EmptyFound > 0)
        document.getElementById("welcomemessage").innerHTML = ErrorsFound + (ErrorsFound > 1 ? " ошибки" : " ошибка") + " и " + EmptyFound + (EmptyFound > 1 ? " неполные слова были" : " неполное слово было") + " найдено(-ы).";
    else if (ErrorsFound > 0)
        document.getElementById("welcomemessage").innerHTML = ErrorsFound + (ErrorsFound > 1 ? " ошибок было найдено." : " ошибка была найдена.");
    else if (EmptyFound > 0)
        document.getElementById("welcomemessage").innerHTML = "Ошибок найдено не было, но " + EmptyFound + (EmptyFound > 1 ? " слова неполностью заполнены" : " слово неполностью заполнено") + ".";
